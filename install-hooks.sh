...
for hook in pre-commit commit-msg; do
    if [ -f "$HOOKS_TARGET_DIR/$hook" ] && [ -x "$HOOKS_TARGET_DIR/$hook" ]; then
        echo -e "   [OK] $hook hook installed and executable"
        hooks_count=$((hooks_count + 1))
    else
        echo -e "   [ERROR] $hook hook installation failed"
    fi
done

# Check Python availability
if command -v python3 >/dev/null 2>&1; then
    echo -e "   [OK] Python 3 is available"
else
    echo -e "   [WARNING] Python 3 not found in PATH"
fi
