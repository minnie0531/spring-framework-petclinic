...
echo -e "${BLUE}===========================================${NC}"
echo -e "${BLUE}Uninstalling AI-Powered Git Hooks${NC}"
echo -e "${BLUE}===========================================${NC}"
...
if [ ! -d "$HOOKS_TARGET_DIR" ]; then
    echo -e "${YELLOW}No hooks directory found. Nothing to uninstall.${NC}"
    exit 0
fi
...
if [ ${#hooks_to_remove[@]} -eq 0 ]; then
    echo -e "${YELLOW}No AI-powered hooks found to remove.${NC}"
    exit 0
fi

# Confirm removal
echo ""
echo -e "${YELLOW}WARNING: This will remove the following hooks:${NC}"
...
for hook in "${hooks_to_remove[@]}"; do
    if rm -f "$HOOKS_TARGET_DIR/$hook"; then
        echo -e "   [OK] Removed: $hook"
        removed_count=$((removed_count + 1))
    else
        echo -e "   [ERROR] Failed to remove: $hook"
    fi
done
